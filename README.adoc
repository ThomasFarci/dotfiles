...
= Overview
This repo contains my config files managed by https://github.com/aspiers/stow[GNU stow], a wonderful tool that symlinks files in a directory to the same locations in your home folder, so you can version control them.
...
----
Note that by default, it links to the parent directory from which stow was run, so not nessarily `$HOME`, but here I privided the file xref:.stowrc[.stowrc] which sets an option to make it the always home folder. 

After the script has run stow, it will check for `scripts/setup-module-<platform>.sh` and run it if it exists.
